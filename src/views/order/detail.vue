<template>
  <div>
    ...
    <div class="house-box padding">
      <span
        class="bold text-primary hold expired"
        v-if="baseOrderInfo && baseOrderInfo.status == 0"
      >已过期</span>
    ...
    <div class="row padding">
      <div class="card">
        ...
        <div class="small-text">
          <span class="bold">押金说明：</span>入住时缴纳金额为总费用40%的押金，离店时补齐剩余费用。
        </div>
        <div class="small-text">
          <span class="bold">发票：</span>如需开票，请在离店时联系房东开票。
        </div>
      </div>
    </div>
    ...
    <van-submit-bar
      ...
      v-show="!showPaymentModal"
      placeholder
      :price="baseOrderInfo.surplus_money"
      label="支付尾款:"
      :button-text="mode=='create'?'立即预定':'立即支付'"
      @submit="handleCreateOrder"
    >
      <template #tip v-if="baseOrderInfo && baseOrderInfo.status==2">
        ...
        >总金额{{(baseOrderInfo.total_money/100).toFixed(2)}}, 已支付定金{{(baseOrderInfo.earnest_money/100).toFixed(2)}}</div>
      </template>
    </van-submit-bar>
  </div>
</template>
...
const tomorrowStr = computed(() => {
  ...
  return (
    padZero(nextDay.getMonth() + 1) + "月" + padZero(nextDay.getDate()) + "日"
  );
});
  ...
  orderDetail({
    ...
    if (res.status == 1) {
      timer = setInterval(() => {
        ...
        let offsetTime =
          new Date(baseOrderInfo.value.create_at).getTime() +
          20 * 60 * 1000 -
          nowTime;
